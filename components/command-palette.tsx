...
export function CommandPalette({ onClose }: CommandPaletteProps) {
  ...
  const files = [
    ...
    },
    {
      ...
      icon: FileCode,
      label: 'efurnish.tsx',
      action: () => router.push('/projects/efurnish')
    },
    {
      id: 8,
      icon: FileCode,
      label: 'intern-trail.tsx',
      action: () => router.push('/projects/intern-trail')
    },
    {
      id: 9,
      icon: FileCode,
      label: 'stay-well.tsx',
      action: () => router.push('/projects/stay-well')
    },
    {
      id: 10,
      icon: File,
    ...
    <div
      ...
      <div
        ...
        <div className="max-h-[50vh] overflow-y-auto">
              ...
              <div className="space-y-1 p-1">
                {filteredCommands.map((cmd, index) => (
                  <div
                    ...
                    <span>{cmd.label}</span>
                    <span className="ml-auto text-xs text-[#bbbbbb]">
                      {cmd.shortcut}
                    </span>
                  </div>
                ))}
              ...
              <div className="space-y-1 p-1">
                {filteredFiles.map((file, index) => (
                  <div
                    key={file.id}
                    className={`flex items-center px-3 py-2 rounded cursor-pointer ${
                      selectedIndex === index + filteredCommands.length
                        ? 'bg-[#37373d]'
                        : 'hover:bg-[#2a2d2e]'
                    }`}
                    onClick={() => {
          ...
          {allItems.length === 0 && (
            <div className="p-4 text-center text-[#bbbbbb]">
